...
function useTokenRefresh(options = {}) {
  ...
  const onExpiredRef = useRef2(onSessionExpired);
  const thresholdRef = useRef2(threshold);
  const endpointRef = useRef2(endpoint);
  useEffect3(() => {
  ...
  const scheduleRef = useRef2();
  const doRefreshRef = useRef2();
